...
export default function BillingPage() {
  ...
  const metrics = [
    { label: 'Monthly Recurring Revenue', value: `${stats?.mrr || 0} TND` },
    { label: 'Active Subscriptions', value: stats?.activeSubscriptions || 0 },
    { label: 'Avg Revenue Per User', value: `${stats?.avgRevenuePerUser || 0} TND` },
  ];
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-20">
      {/* Header */}
      <div className="mb-10">
        <h1 className="text-[32px] font-bold text-white tracking-tight mb-2">Billing & Subscriptions</h1>
      ...
      <div className="glass-card p-8">
        <h2 className="text-[16px] font-bold text-white mb-8 tracking-wide">Subscription Tier Distribution</h2>
        <div className="space-y-8 max-w-3xl">
          <div>
            <div className="flex justify-between items-end mb-3">
              ...
              <span className="text-white font-bold text-[15px]">{stats?.distribution?.Enterprise || stats?.distribution?.Premium || 0} Orgs</span>
            </div>
          ...
          <div>
            <div className="flex justify-between items-end mb-3">
              ...
              <span className="text-white font-bold text-[15px]">{stats?.distribution?.Pro || 0} Orgs</span>
            </div>
            <div className="h-3 w-full rounded-full bg-black/40 border border-white/5 overflow-hidden">
          ...
          <div>
            <div className="flex justify-between items-end mb-3">
              ...
              <span className="text-white font-bold text-[15px]">{stats?.distribution?.Basic || 0} Orgs</span>
            </div>
            <div className="h-3 w-full rounded-full bg-black/40 border border-white/5 overflow-hidden">
      ...
      <div className="mt-12 glass-card overflow-hidden">
        <div className="p-8 border-b border-white/[0.06] flex justify-between items-center bg-white/[0.01]">
          <div>
            <h2 className="text-[16px] font-bold text-white tracking-wide">Active Organizational Subscriptions</h2>
            <p className="text-[#A69697] text-[13px] mt-1">A detailed list of all tenant organizations and their current billing plans.</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.02]">
                <th className="py-4 px-8 font-semibold text-[11px] uppercase tracking-widest text-[#A69697] border-b border-white/[0.06]">Organization</th>
                <th className="py-4 px-8 font-semibold text-[11px] uppercase tracking-widest text-[#A69697] border-b border-white/[0.06]">Contact Email</th>
                <th className="py-4 px-8 font-semibold text-[11px] uppercase tracking-widest text-[#A69697] border-b border-white/[0.06]">Subscription Tier</th>
                <th className="py-4 px-8 font-semibold text-[11px] uppercase tracking-widest text-[#A69697] border-b border-white/[0.06] text-right">MRR Contribution</th>
              </tr>
            </thead>
            <tbody>
                    ...
                    <tr key={i} className="group hover:bg-white/[0.03] transition-colors border-b border-white/[0.04] last:border-0 cursor-pointer">
                      ...
                      <td className="py-5 px-8">
                        <span className={`inline-flex items-center px-3 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-wider border ${
                          ...
                          org.plan?.includes('Pro') ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-white/5 text-[#A69697] border-white/10'
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-full mr-2 ${
                            org.plan?.includes('Premium') || org.plan?.includes('Primum') || org.plan?.includes('Enterprise') ? 'bg-[#D98F8F]' : 
                            org.plan?.includes('Pro') ? 'bg-blue-400' : 'bg-[#A69697]'
                          }`}></span>
                          {org.plan}
                        </span>
                      </td>
                      <td className="py-5 px-8 text-right">
                        ...
                        <p className="text-[11px] text-[#A69697] mt-0.5 uppercase tracking-wider">Per Month</p>
                      </td>
                    </tr>
                  );
                })
