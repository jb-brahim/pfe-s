...
export default function MessagesPage() {
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col h-[calc(100vh-120px)] w-full max-w-[1200px] mx-auto bg-[rgba(255,255,255,0.02)] backdrop-blur-md border border-[rgba(255,255,255,0.05)] rounded-[24px] shadow-lg overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Sidebar */}
          <div className="w-[200px] border-r border-white/5 flex flex-col p-4 gap-2">
            <button
                ...
                <div className="flex justify-between items-center mb-1">
                  ...
                  {user?.role === 'ADMIN' && (
                    <button 
                      type="button"
                      onClick={() => {
                        const allEmployeeIds = contacts.filter(c => c.role === 'ACCOUNTANT').map(c => c._id);
                        if (composeForm.receiverIds.length === allEmployeeIds.length) {
                          setComposeForm({ ...composeForm, receiverIds: [] });
                        } else {
                          setComposeForm({ ...composeForm, receiverIds: allEmployeeIds });
                        }
                      }}
